(function () {
  ...
  var GameView = Tetris.GameView = function (ctx) {
    ...
    // this.$canvas.on("tap", this.start.bind(this));
  ...
  GameView.prototype.bindSwipeHandlers = function () {
    ...
    //   var action = GameView.KEYBINDS[keyPressed];
    //   this.$canvas.on(keyPressed, function () {game.safeAction(action);})
    // }
    this.$canvas = $("*");

    this.$canvas.on("swiperight", function (event) {alert(event.currentTarget); game.safeAction("moveRight");})
    this.$canvas.on("tap", function (event) {alert(event.currentTarget); game.safeAction("rotate");})
    this.$canvas.on("swipeleft", function (event) {alert(event.currentTarget); game.safeAction("moveLeft");})
    this.$canvas.on("taphold", function (event) {alert(event.currentTarget); game.dropPiece();})
  };
  ...
  GameView.prototype.unBindSwipeHandlers = function () {
    ...
    this.$canvas.on("tap", function () {this.newGame()}.bind(this));
  };
